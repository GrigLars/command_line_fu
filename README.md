...
[Logging inside a script to an external log](####Logging inside a script to an external log)
[Some tips on connecting a dumb terminal to modern systemd Linux](####Some tips on connecting a dumb terminal to modern systemd Linux)
[Using escape characters in bash "read" statements](####Using escape characters in bash "read" statements)
[Using ssh-agent to log in without having to do "-i" at every commandline](####Using ssh-agent to log in without having to do "-i" at every commandline)
[Rescuing an LVM volume/drive off a second rescue system:](####Rescuing an LVM volume/drive off a second rescue system:)
[The chrony equivalent to the ntpdate SERVER-ADDRESS command is:](####The chrony equivalent to the ntpdate SERVER-ADDRESS command is:)
[Remove lines using sed](####Remove lines using sed)
[rsync using ssh key](####rsync using ssh key)
[Using ansible vault encryption via the command line, the short aliased version](####Using ansible vault encryption via the command line, the short aliased version)
[Base64 obfuscation ](####Base64 obfuscation )
[Sort text file and remove duplicates in place](####Sort text file and remove duplicates in place)
[Remove duplicates in a text file without sorting first](####Remove duplicates in a text file without sorting first)
[Find your IP on the command line](####Find your IP on the command line)
[Making a RAM disk to install an entire OS guest into memory](####Making a RAM disk to install an entire OS guest into memory)
[Quick mount of a samba/smb/cifs share ####](####Quick mount of a samba/smb/cifs share ####)
[Visual readout of processes ####](####Visual readout of processes ####)
[Adding a user with no password ####](####Adding a user with no password ####)
[Quick Guide to expanding disk size on VMware with Debian/Ubuntu and hoping it's using LVM ####](####Quick Guide to expanding disk size on VMware with Debian/Ubuntu and hoping it's using LVM ####)
[Quick Guide to expanding EBS disk size on AWS with Debian/Ubuntu [LVM not needed] ####](####Quick Guide to expanding EBS disk size on AWS with Debian/Ubuntu [LVM not needed] ####)
[Quick Guide to expanding EBS disk size on AWS with Debian/Ubuntu with XFS ####](####Quick Guide to expanding EBS disk size on AWS with Debian/Ubuntu with XFS ####)
[Removing files over X days old ####](####Removing files over X days old ####)
[Changing your sudoers file to lecture differently ####](####Changing your sudoers file to lecture differently ####)
[Using awk to show lines that have a match in a column ####](####Using awk to show lines that have a match in a column ####)
[Regex for email and ip addresses](####Regex for email and ip addresses)
[Getting keys and values from bash associative arrays](####Getting keys and values from bash associative arrays)
[How to make a sudoer be able to sudo with no password](####How to make a sudoer be able to sudo with no password)
[You can substitute command output for files](####You can substitute command output for files)
[Move files with ad-hoc web server](####Move files with ad-hoc web server)
[Read JSON files in a sane way](####Read JSON files in a sane way)
[How to connect to an ansible host NOT in inventory (yet)](####How to connect to an ansible host NOT in inventory (yet))
[Have bash determine what quotes to use](####Have bash determine what quotes to use)
[Update and remove all stale vagrant boxes](####Update and remove all stale vagrant boxes)
[Searching for all active crontab jobs](####Searching for all active crontab jobs)
[How to List All Running Services Under systemd ](####How to List All Running Services Under systemd )
[How to run a command for a certain length of time](####How to run a command for a certain length of time)
[Am I running Raspbian 32 bit or 64 bit?](####Am I running Raspbian 32 bit or 64 bit?)
[Check what SSL/TLS client connection you get](####Check what SSL/TLS client connection you get)
[Put your last long command in your editor](####Put your last long command in your editor)
[How to measure GPU and CPU temp on a Raspberry Pi](####How to measure GPU and CPU temp on a Raspberry Pi)
[More on stdout and stderr](####More on stdout and stderr)
[How to grep docker logs](####How to grep docker logs)
[How to show “Hybrid” line numbers in Vim](####How to show “Hybrid” line numbers in Vim)
[For Excel or CSV files](####For Excel or CSV files)
[Ah, crap, I didn't mean to type that long command yet, but I don't want to retype it](####Ah, crap, I didn't mean to type that long command yet, but I don't want to retype it)
[Some cool hardware reading tricks](####Some cool hardware reading tricks)
[How do I prevent a cron job from running over itself if it takes too long?](####How do I prevent a cron job from running over itself if it takes too long?)
[colorize log output](####colorize log output)
[remove trailing whitespace](####remove trailing whitespace)
[telnet exit](####telnet exit)
[kwallet issues](####kwallet issues)
[Something I learned about k3b files](####Something I learned about k3b files)
[A rare Windows one: get all the system info in a dump](####A rare Windows one: get all the system info in a dump)
[Seeing a conf file without comments or blank spaces](####Seeing a conf file without comments or blank spaces)
[Disabling vagrant-guest plugin for a weird vbox](####Disabling vagrant-guest plugin for a weird vbox)
[sort IP addresses in a sane way](####sort IP addresses in a sane way)
[awk "print everything past the first two columns" ](####awk "print everything past the first two columns" )
[Some git stuff I forget](####Some git stuff I forget)
[Change default browser launched on command line in Ubuntu](####Change default browser launched on command line in Ubuntu)
[Add timeout option to read command, but in case of interactive execution, user can easily skip waiting.](####Add timeout option to read command, but in case of interactive execution, user can easily skip waiting.)
[AURGH which is it, adduser or useradd?](####AURGH which is it, adduser or useradd?)
[Make sure you are root in a script](####Make sure you are root in a script)
[Basic IPtables stuff](####Basic IPtables stuff)
[Network tools ](####Network tools )
[Start a process on a different tty:](####Start a process on a different tty:)
[Windows instead of snipping tool shortcut](####Windows instead of snipping tool shortcut)
[Show what inputs are attached and to disable touchpad on Levono laptops with Ubuntu](####Show what inputs are attached and to disable touchpad on Levono laptops with Ubuntu)
[Un-ban someone in a running "fail2ban" setup](####Un-ban someone in a running "fail2ban" setup)
[Ansible notes:](####Ansible notes:)
[That software that has variables for docker in key/value pairs](####That software that has variables for docker in key/value pairs)
[Unix permissions: dealing with my dyslexia ](####Unix permissions: dealing with my dyslexia )
[Access control lists:](####Access control lists:)
[VirtualBox management on command line:](####VirtualBox management on command line:)
[To change the default editor in Ubuntu](####To change the default editor in Ubuntu)
[That key you like (shorter than the rest, more secure for now):](####That key you like (shorter than the rest, more secure for now):)
[Command line shortcuts ](####Command line shortcuts )
[Gives you vi functionality on the command line.](####Gives you vi functionality on the command line.)
[memory info](####memory info)
[strace will tell you the path of every file the program tries to open.](####strace will tell you the path of every file the program tries to open.)
[Find all logs which have changed within 5 minutes](####Find all logs which have changed within 5 minutes)
[Sometimes instead of netcat you can use socat. Sure it isn't as simple but has a lot more features.](####Sometimes instead of netcat you can use socat. Sure it isn't as simple but has a lot more features.)
[Packages you like (note, some may be named different in deb vs. rpm packages):](####Packages you like (note, some may be named different in deb vs. rpm packages):)
[To check an ssh port on SELinux (you need the policycoreutils-python package):](####To check an ssh port on SELinux (you need the policycoreutils-python package):)
[To add an ssh port on SELinux:](####To add an ssh port on SELinux:)
[Mailing when people login](####Mailing when people login)
[show context in grep 2 lines before AND after](####show context in grep 2 lines before AND after)
[Output to clipboard](####Output to clipboard)
[For logs that get rotated, you may want tail -F or tail --follow=name so you start following the new log rather than the old file that got moved out.](####For logs that get rotated, you may want tail -F or tail --follow=name so you start following the new log rather than the old file that got moved out.)
[You can also tail -f multiple files simultaneously!  Package multitail also works (with color, too):](####You can also tail -f multiple files simultaneously!  Package multitail also works (with color, too):)
[To terminate an ssh session that's hung or something, type this sequence ](####To terminate an ssh session that's hung or something, type this sequence )
[Put in your .bashrc or /etc/bash.bashrc for history control](####Put in your .bashrc or /etc/bash.bashrc for history control)
[awk using multiple field separators, use pipe:](####awk using multiple field separators, use pipe:)
[Save a file you edited in vim without the needed permissions](####Save a file you edited in vim without the needed permissions)
[edit a command in the editor](####edit a command in the editor)
[Runs previous command replacing foo by bar every time that foo appears](####Runs previous command replacing foo by bar every time that foo appears)
[Put a console clock in top right corner	](####Put a console clock in top right corner	)
[Glob expanding](####Glob expanding)
[Terminal emulation](####Terminal emulation)
[Replace a whole line on one match (note "dot star" at beginning and end)](####Replace a whole line on one match (note "dot star" at beginning and end))
[](####)
[Delete a whole line on match](####Delete a whole line on match)
[How to I write to dev null again? I always screw the order up in crontab reports.](####How to I write to dev null again? I always screw the order up in crontab reports.)
[Check if something is in a bash array:](####Check if something is in a bash array:)
[Do some stuff with the console (helpful with virsh console [guest]) in Linux KVM](####Do some stuff with the console (helpful with virsh console [guest]) in Linux KVM)
[Change the text and other Screensaver properties in KDE:](####Change the text and other Screensaver properties in KDE:)
[yum groups](####yum groups)
[List a lot of cronbtabs at once:](####List a lot of cronbtabs at once:)
[ASCII "Video" you keep forgetting:](####ASCII "Video" you keep forgetting:)
[snmpd.conf ](####snmpd.conf )
[Install XFCE on CentOS 6](####Install XFCE on CentOS 6)
[set in bash](####set in bash)
[bash sequencing](####bash sequencing)
[bash sequencing with seq auto-adjusting for width](####bash sequencing with seq auto-adjusting for width)
[String maniuplation in bash](####String maniuplation in bash)
[Enable PCRE to group in grep](####Enable PCRE to group in grep)
[Human copy with progress bar	](####Human copy with progress bar	)
[Generate various SSL certificate, when a CSR has to be sent to a signing authority, which also creates new key:](####Generate various SSL certificate, when a CSR has to be sent to a signing authority, which also creates new key:)
[This is a quick step to generate self-signed certificate :](####This is a quick step to generate self-signed certificate :)
[I have a keypair. How do I determine the key length?](####I have a keypair. How do I determine the key length?)
[How do I compare a key to a cert?](####How do I compare a key to a cert?)
[Get expiration of certificate for various protocols: file, https, pop3, imap, smtps, starttls](####Get expiration of certificate for various protocols: file, https, pop3, imap, smtps, starttls)
[Print out separate colors for each output	](####Print out separate colors for each output	)
[Expand via curly braces:](####Expand via curly braces:)
[My rsync reminder (because I always get confused)](####My rsync reminder (because I always get confused))
[Formatting time display](####Formatting time display)
[BASH Auto complete](####BASH Auto complete)
[READ LINES WITH SPACES AND STUFF](####READ LINES WITH SPACES AND STUFF)
[You can also read field wise:](####You can also read field wise:)
[To lock out a user account.  Several ways:](####To lock out a user account.  Several ways:)
[To check an ssh port on SELinux (you need the policycoreutils-python package):](####To check an ssh port on SELinux (you need the policycoreutils-python package):)
[To add an ssh port on SELinux:](####To add an ssh port on SELinux:)
[Make a 4gb swap file](####Make a 4gb swap file)
